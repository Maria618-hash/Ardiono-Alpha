...
\begin{frame}{Outline}
  ...
  \begin{tikzpicture}[node distance=6mm]
    \node (a) [outlinebox] {\faThumbsUp\hspace{0.6em} Introduction};
    \node (b) [outlinebox, below=of a] {\faBook\hspace{0.6em} Literature Review};
    \node (c) [outlinebox, below=of b] {\faPencilAlt\hspace{0.6em} Methodology};
    \node (d) [outlinebox, below=of c] {\faClock\hspace{0.6em} Experiments and Results};
    \node (e) [outlinebox, below=of d] {\faChartLine\hspace{0.6em} Conclusion};
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Motivation}
  \begin{itemize}
    \item Debug and programming interfaces provide invasive control.
    \item Host-side policies can be bypassed after reflashing.
    \item Authorization should be enforced at the device execution model.
  \end{itemize}
\end{frame}
...
\begin{frame}{Literature Review (Summary Table)}
  ...
  \begin{tabular}{@{}p{0.7cm}p{3.4cm}p{4.4cm}p{3.2cm}@{}}
    \rowcolor{gray!20}
    \textbf{Ref.} & \textbf{Area} & \textbf{Representative focus} & \textbf{Gap / Our angle} \\
    1 & Secure boot & BootROM validation and measured boot chains & Post-boot debug remains open \\
    2 & Debug/JTAG auth & Interface protection and authentication & Not tied to execution state \\
    3 & TEEs/minimal TCB & Isolation of trusted code & Debug gating not enforced \\
    4 & Attestation/CFI & Runtime integrity evidence & Evidence without access control \\
    5 & Toolchain/ISA & Extensible ISA + compiler support & Needs device-local authorization \\
  \end{tabular}
  \vspace{0.6ex}
  \begin{itemize}
    \item \textbf{This work:} ISA-triggered, key-gated authorization enforced on-device.
  \end{itemize}
\end{frame}
...
\begin{frame}{Threat Model}
  \begin{itemize}
    \item Adversary has physical access to the device.
    \item Attacker can probe debug/program interfaces.
    \item Goal is to prevent unauthorized execution and debug enablement.
  \end{itemize}
\end{frame}
...
\begin{frame}{Implementation Overview}
  \begin{itemize}
    \item ISA integration: decode path emits \texttt{kc\_event}.
    \item Protected NVM: key not memory-mapped.
    \item Comparator + FSM: constant-time match and gating.
    \item Host workflow: compile, authorize, program, log.
  \end{itemize}
\end{frame}
...
\begin{frame}{Limitations}
  \begin{itemize}
    \item Evaluation emphasizes functional evidence over performance metrics.
    \item Side-channel and fault-injection resilience not fully quantified.
    \item Deployment requires formal provisioning and recovery policy.
  \end{itemize}
\end{frame}
...
\begin{frame}{Figure References}
  \begin{itemize}
    \item Architecture diagram and flowchart: this work (device and host workflow).
    \item Toolchain/HEX/authorization screenshots: generated from the prototype toolchain.
    \item Debug and programming logs: captured from host application runs.
  \end{itemize}
\end{frame}
