...
\begin{figure}[t]
  ...
  \end{tikzpicture}
  \caption{Authorization flow and gating logic (flowchart).}
  \label{fig:auth-flow}
\end{figure}
...
The Authenticator FSM is defined by:
{\small
\[
\mathbf{S}=\{\textsf{LOCKED},\textsf{VERIFY},\textsf{AUTHORIZED},\textsf{HALT}\},\quad
\mathbf{I}=\{\texttt{kc\_event},\texttt{match}\},\quad
\mathbf{O}=\{\texttt{exec\_enable},\texttt{debug\_enable},\texttt{halt\_cpu}\}.
\]
}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.95\textwidth]{fig_arch_flow.png}
