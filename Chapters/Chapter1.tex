...
Embedded platforms commonly operate with constrained resources, long device lifetimes,
and limited update channels. These properties increase the impact of unauthorized
reflashing and debugging because a single compromise can persist for long periods. At
the same time, boot-time integrity mechanisms alone do not necessarily control
privileged actions after the system begins executing normal firmware. Device-resident
policies that gate execution and debug enablement are therefore essential for resilient
operation in physical-access threat models. This motivates security mechanisms that are
anchored in hardware, integrated into the execution model, and enforceable independent
of host-side tooling.
...
The threat model assumes physical access to the device, including the ability to attach
debug tools, observe firmware images, and attempt unauthorized reprogramming. The
attacker is assumed to have the capability to replace or modify firmware unless
explicitly prevented by on-device checks. The security requirements therefore include:
device-local authorization prior to releasing execution and debug capability, fail-stop
behavior on authorization failure, and key storage that is inaccessible via standard
memory transactions. These requirements guide both the hardware datapath and the
...
The evaluation scope emphasizes functional correctness and end-to-end enforceability,
using build artifacts and host workflow logs to demonstrate policy compliance. Detailed
area/power overheads and formal certification are left for future work.
...
\begin{itemize}
  \item How can device-local authorization be enforced in a way that is visible in the
        execution model and independent of host-side policy?
  \item What minimal ISA extension is sufficient to trigger secure authorization while
        preserving toolchain compatibility?
  \item How can protected key storage and a fail-stop FSM be integrated to gate both
        execution and debug enablement?
  \item What evidence is sufficient to demonstrate end-to-end enforceability in a
        practical embedded workflow?
\end{itemize}

\section{Methodology Overview}
The methodology integrates hardware, firmware, and toolchain components. A custom
instruction (\texttt{kc}) is added to the ISA, and the decode/execute path forwards
operands to a constant-time verification unit. Key material is provisioned into
protected non-volatile storage that is not memory-mapped. An Authenticator FSM gates
execution and debug enablement based on verification outcomes. On the host side, the
toolchain emits the authorization primitive and the application performs compilation,
inspection, key exchange, and programming. Chapter~3 details the architecture and
protocol, while Chapters~4 and~5 describe implementation and evaluation evidence.
